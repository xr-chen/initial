...
class pipeline(object):
    ...
    def __init__(self):
        ...
        self.pineUp = pygame.image.load(r"assets/top.png")
        self.pineDown = pygame.image.load(r"assets/bottom.png")
...
def createMap():
    ...
    # 显示管道
    screen.blit(Pipeline.pineUp, (Pipeline.wallx,h1))   # 上管道坐标位置
    screen.blit(Pipeline.pineDown, (Pipeline.wallx,h2))  # 下管道坐标位置
...
def checkDead():
    # 上方管子的矩形位置
    upRect = pygame.Rect(Pipeline.wallx,h1,
                         Pipeline.pineUp.get_width(),
                         Pipeline.pineUp.get_height())

    # 下方管子的矩形位置
    downRect = pygame.Rect(Pipeline.wallx,h2,
...
if __name__ == '__main__':
    ...
    while a=='OK':
        ...
        while stt:
            ...
            background = pygame.image.load(r"assets/backgroud.jpg")  # 加载背景图片
            if checkDead():                      # 检测小鸟生命状态
                getResutl()                      # 如果小鸟死亡，显示游戏总分数
                a=easygui.msgbox('Continue?', 'Game Over')
                del(Bird)
                del(Pipeline)
                del(font)
                del(screen)
